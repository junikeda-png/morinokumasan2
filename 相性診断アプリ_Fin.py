...
# 性格タイプリストの構築
personality_options = ["あなたの性格タイプを選択してください"] + list(personality_descriptions.keys())

# 画像が格納されているフォルダの指定
image_dir = os.path.join(os.getcwd(), "image")  # "image" フォルダを指定

#アプリ名
st.sidebar.markdown("### 相性診断アプリ")

# サイドバー: ユーザー情報入力
user_personality = st.sidebar.selectbox("あなたの性格タイプを選択してください", options=personality_options, index=0)


# サイドバーで自分の性格タイプを選択したときに対応する画像を表示
if user_personality != "あなたの性格タイプを選択してください":
...
if user_age == 23 and user_personality == "ENFJ - 主人公":
    content_styles.insert(0, "りじちょー")  # 一番上に追加
